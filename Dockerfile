...
ENV VIRTUAL_ENV /opt/ink-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/ink/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/ink/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/ink/ && touch src/ink/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/ink/
...
# Expose the app.
ENTRYPOINT ["/opt/ink-env/bin/ink"]
CMD []
